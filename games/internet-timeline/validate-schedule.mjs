...
const roles = read('gameplay-roles.json');

const legacy = [...base, ...extra];
const all = [...base, ...extra, ...fun];
const byId = new Map(all.map((q) => [q.id, q]));
const specialist = new Set(curation.specialist || []);
const retired = new Set(curation.retire || []);
const anchorRole = new Set(roles.anchor || []);
const confusionRole = new Set(roles.confusion || []);
const surpriseRole = new Set(roles.surprise || []);
const stretchRole = new Set(roles.stretch || []);
...
const SCHEDULE_VERSION = 'schedule-v7';
...
function legacyDifficulty(q){if(q.difficulty)return q.difficulty;if(q.answerYear<1988)return'hard';if(q.answerYear>=2001&&['Social','Gaming','AI','Mobile','Entertainment','Culture'].includes(q.category))return'easy';if(['Security','Web'].includes(q.category)&&q.answerYear<1995)return'hard';return'medium'}
function difficultyOf(q){if(meta(q).difficulty)return meta(q).difficulty;return legacyDifficulty(q)}
...
function explicitRolePenalty(q,role){if(role==='anchor')return anchorRole.has(q.id)?-220:90;if(role==='warmup')return anchorRole.has(q.id)?-90:0;if(role==='memory'||role==='tension')return confusionRole.has(q.id)?-190:80;if(role==='surprise')return surpriseRole.has(q.id)?-260:130;if(role==='stretch')return stretchRole.has(q.id)?-210:confusionRole.has(q.id)?-70:70;return 0}
function rolePenalty(q,role,p){const appeal=appealOf(q),difficulty=difficultyOf(q);let v=explicitRolePenalty(q,role);if(role==='anchor'){if(appeal!=='mainstream')v+=appeal==='known'?180:650;if(difficulty==='hard')v+=700}if(role==='warmup'){if(appeal==='niche')v+=600;if(difficulty==='hard')v+=600}if(role==='memory'&&appeal==='niche')v+=350;if(role==='surprise'){if(!isFun(q))v+=240;if(appeal==='niche')v+=300}if(role==='tension'){if(difficulty==='easy')v+=80;if(appeal==='niche')v+=240}if(role==='stretch'&&p.key!=='hard'&&appeal==='niche')v+=400;if(p.key!=='hard'&&difficulty==='hard'&&role!=='stretch')v+=450;return v}
...
function legacyCandidateScore(q,selected,dayIndex,slot,p){const tie=hashSeed(`${GAME_ID}:schedule-v4:${dayIndex}:${slot}:${q.id}:${p.key}`)/0xffffffff;const target=['medium','easy','medium','easy','hard','medium'][slot]??'medium';const difficultyPenalty=legacyDifficulty(q)===target?0:220;const sameYearPenalty=selected.some(x=>x.answerYear===q.answerYear)?10000:0;const eraCount=selected.filter(x=>eraBucket(x.answerYear)===eraBucket(q.answerYear)).length;const categoryCount=selected.filter(x=>x.category===q.category).length;const minDist=selected.length?Math.min(...selected.map(x=>Math.abs(x.answerYear-q.answerYear))):50;const distanceTerm=p.close?Math.min(minDist,20)*1.8:-Math.min(minDist,20)*1.1;return sameYearPenalty+profilePenalty(q,p)+difficultyPenalty+categoryCount*92+eraCount*24+distanceTerm+tie}
...
function legacyBuildDeck(dayIndex,recentDays,profile){const blocked=new Set(recentDays.flat().map(q=>q.id)),available=legacy.filter(q=>!blocked.has(q.id)),pool=available.length>=CARDS_NEEDED?available:legacy,selected=[];for(let slot=0;slot<CARDS_NEEDED;slot++){const unique=pool.filter(q=>!selected.some(x=>x.id===q.id||x.answerYear===q.answerYear));const remaining=(unique.length?unique:pool.filter(q=>!selected.some(x=>x.id===q.id))).sort((a,b)=>legacyCandidateScore(a,selected,dayIndex,slot,profile)-legacyCandidateScore(b,selected,dayIndex,slot,profile));if(!remaining[0])throw new Error('Not enough legacy questions');selected.push(remaining[0])}return selected}
...
for(let i=0;i<LEGACY_DAYS+60;i++){
  ...
  const deck=i<LEGACY_DAYS?legacyBuildDeck(i,recent,p):buildDeck(i,recent,p);
  scheduled.push(deck);
  ...
  const confusionCount=deck.filter(q=>confusionRole.has(q.id)).length;
  const surpriseCount=deck.filter(q=>surpriseRole.has(q.id)).length;
  ...
  if(!anchorRole.has(deck[0].id))problems.push(`#${n}: anchor slot is not an explicit anchor (${deck[0].id}) :: ${summary}`);
  if(confusionCount<2)problems.push(`#${n}: only ${confusionCount} confusion cards :: ${summary}`);
  if(surpriseCount<1)problems.push(`#${n}: no explicit surprise card :: ${summary}`);
